...
- Initialise the data (requires [`psp-cli`](https://github.com/Portfolio-Solver-Platform/psp-cli) and `jq`):
    ```bash
    psp config set client_id admin-app
    psp auth login
    ./post-data-setup.sh
    ```
    To override the solver image: `MINIZINC_SOLVERS_IMAGE=<url> ./post-data-setup.sh`
